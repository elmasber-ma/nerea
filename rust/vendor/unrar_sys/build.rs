fn main() {
    // PATCH mimapp: en Android no existe libpthread (va dentro de libc);
    // linkear -lpthread rompe el build con NDK. Usamos CARGO_CFG_TARGET_OS
    // porque cfg!() en build scripts evalua el HOST, no el target.
    let target_os = std::env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
    let es_android = target_os == "android";
    if cfg!(windows) {
        println!("cargo:rustc-flags=-lpowrprof");
        println!("cargo:rustc-link-lib=shell32");
        if !es_android && cfg!(target_env = "gnu") {
